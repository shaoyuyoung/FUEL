...
class FuzzingCore:
    ...
    def execute_single_round(self, feedback_data, heuristic, op_nums, diff_type):
        ...
        if isinstance(als_prompt_or_text, str) and als_prompt_or_text != "use generation by default":
            als_text = self.als_model.analyze(
                role=self.prompt_handler.als_prompt_config["system_content"].format(lib=self.lib),
        ...
        gen_text = self.gen_model.generate(
            role=self.prompt_handler.gen_prompt_config["system_content"].format(lib=self.lib), 
            prompt=gen_prompt
        )
