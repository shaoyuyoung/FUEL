...
class ModelManager:
    ...
    @staticmethod
    def setup_models(gen_model_config=None, als_model_config=None, 
                    gen_model_path=None, als_model_path=None):
        """
        ...
        if not gen_config_path or not als_config_path:
            raise ValueError("Must provide configuration file paths for both generation and analysis models")
        
        # Check if configuration files exist
        if not os.path.exists(gen_config_path):
            raise FileNotFoundError(f"Generation model configuration file not found: {gen_config_path}")
        if not os.path.exists(als_config_path):
            raise FileNotFoundError(f"Analysis model configuration file not found: {als_config_path}")
